...
def add_pos_tags(sentences):
    ...
    for sent, tagged in zip(sentences, tagged_sents):
        for item, (_, pos) in zip(sent, tagged):
            item["pos"] = pos
...
def token_features(sent, i):
    ...
    feats = {
        ...
        # Group C: POS features
        "C:pos": pos,
        "C:pos_prefix1": pos[:1],
        "C:pos_prefix2": pos[:2],

        # Group F: rule/stage2 feature if available
...
def main():
    ...
    parser.add_argument("--output", default="data/splits/test_crf_pos_pred.conll")
    parser.add_argument("--model_out", default="models/crf_pos.pkl")
    ...
    train_dev_sents = add_pos_tags(train_dev_sents)
    ...
    print(f"Training POS-augmented CRF with c1={args.c1}, c2={args.c2}...")
